...
class RepoManager:
    ...
    def init_repo(self, path, username, repo, token=None):
        ...
        if not os.path.exists(os.path.join(path,'.git')):
            ...
            # Ensure we use 'master' as requested for this repo
            try:
                repo_obj.git.branch("-M", "master")
            except:
                pass
        else:
            repo_obj = Repo(path)
        ...
        # Initial commit and Force Push to establish the link and clear remote history
        try:
            ...
            # Force push to master to resolve conflicts with existing remote files
            ...
            if repo.is_dirty(untracked_files=True) or len(repo.untracked_files) > 0:
                ...
                try:
                    ...
                    try:
                        ...
                        # Fallback: Force push to bypass merge conflicts as requested
